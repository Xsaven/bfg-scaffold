    ...
    <v-container>
        <v-simple-table v-if="fields && fields.length">
            <template v-slot:default>
                ...
                <draggable v-model="fields" tag="tbody" handle=".handle">
                <tr v-for="(item, i) in fields" :key="`${model.id}_field_${i}_${item.id}`">
                    ...
                    <td>
                        <json-editor-element v-model="item.methods" @change="changed" :defaults="default_methods" :key="`field_js_editor_${item.name}`" />
                    </td>
                    <td>
                        <v-tooltip bottom>
                            <template v-slot:activator="{ on, attrs }">
